defmodule Bingo do
  ...
  def play!({numbers_to_call, cards}) do
  ...
  defp evaluate_turns(numbers_to_call, cards) do
    Enum.reduce_while(numbers_to_call, cards, fn ({_, index}, cards) ->
      ...
      if index < 5 do
        ...
        case evaluate_board(cards, numbers_called) do
          {:incomplete, _}    -> {:cont, cards}
          {:complete, result} -> {:halt, {:ok, result}}
        end
      end
    end)
  end
  ...
  defp evaluate_board(cards, numbers_called) do
    ...
    case winning_cards do
      ...
      [winner] -> {:complete,   {winner, numbers_called}}
    end
  end
